...
1. 获取账单，这可能需要模拟登录银行或邮箱页面，然后跳到账单页下载下来，因为我每天都会检查邮件，
账单日也不例外，所以我并没写脚本自动获取，顺手就下载了
1. 解析账单数据，这通常需要会一些正则和/或 HTML 解析知识，而后者一般可以用 python 的 beautilful soup 库去解决
...
1. 校对和填充，到这步编码工作已经完成，这步是指导入后的校对工作，一般导入时我都会将交易设为未确认("!")，
校对过程也是把"!"改为"\*"的过程，并且补充上一些细节，这可能需要翻翻购买记录

考虑到记账可能会伴随你的一生，花个一下午写个好用的脚本还是值得的，或者到 [github.com](https://github.com/search)上搜一下，
有些现成的导入某个银行的账单的脚本。

## 旅行记账
在一生中总会有些大事发生，比如外出旅行，买房装修等等，这时会出现一大笔开销，但发现频度很低，
一年几次甚至几年一次，但这会影响你的月平均消费。
因为旅行是一个相对发生更频繁的事，所以这里我举例来说明旅行如何记账。这章的灵感来自于[这里][1]。

### 建立旅行账本
首先是新建一个文本文件，这就是你的旅行账本，这个账本相比主账本会少很多账目，
你可以从主账本里挑一些可能会用到的账目(Account name)进来，在旅行过程中有可能会根据实际情况添加一些进去。
接着要初始化用到的账目，比如银行卡，信用卡，如实记录目前的余额。还可能有一些提前兑换的外币也要记录，
有空的话也可以顺便记录下当前的汇率，在之后成为分析的参考。

有一个比较让人纠结的问题是，有时候计划一次旅行，是从很早以前就开始制定的，可能会提前几个月甚至半年，
...
然后将这几个总数一次写入主账本，并打上个 tag，比如`#trip-some-place-2016`[^1]。

## 建立自己的「梦想」基金
作为一名游戏玩家，时不时需要为信仰充个值，但也不能过度消费，所以我为自己设立了一个「游戏」基金。
买游戏只能使用基金中的钱，如果基金没钱了就不能再买了。
...
![shut up and take my money](/images/shutup.jpg)
...
可以看出我趁打折入了《下一回合 V》。现在检查下基金状况，-100+23=-77，
也就是说我还有 77 块可以用来买入游戏，这你也可以直接统计`Expenses:Games`来获得。

### 小遗憾
这种办法唯一的小遗憾是在用信用卡买游戏上，因为账单导入总是延迟记录已经发生的一切，
所以有可能这个月已经买了许多游戏却无法实时地知道「游戏」基金余额还有多少，我只能自己手动记录已经买的游戏价格，
再与总数相减。最终我决定不管它了，我会对我的当前基金健康状况有个把握，
即使在账单日发现真的超出了预算也没关系，只能在下个月回血让它再度正常了，而我超额的越多，
需要恢复的时间就越长，也就提醒我一定要理智消费，毕竟设立这个基金的目的也是为了控制消费欲望。

## Query
我多次强调记账的目的是为了分析自己的财务状况，而之前提到的 fava 或者 beancount 原生的页面已经能很好的展示一些报表了。
但对于一些特殊需求就需要手写 SQL 来解决了，比如我需要同时知道上个月的收入与支出。

beancount 的 SQL 与 MySQL 等的语法比较相似，它只有一张表，所以 from 只用来指定时间，
剩下的只要搞清楚实际会有哪些列以及一些函数就行了，具体语法可以参考[这里][2]。
...
|tags       |tag 集合，是一个set   |trip-some-place-2016|
...
### 备份
备份我折腾了好久，后来采取定时同步到云盘 + git 的方法管理，同步云盘简单粗暴，
但现在这个环境下不知道哪天哪个盘又倒了，所以还是找个靠谱的国外云盘吧。git 可以同步一些配套的脚本，
但账本属于比较隐私的东西，就不适合放在 github 的 public repo 了，开个 private repo 又不值当，
这种可以自己搞个 gitlab 或者国内的类似产品。

### 运行在云上
之前在网上找教程时找到了[这篇][4]，其中介绍了如何将 fava 部署在 [pythonanywhere](https://www.pythonanywhere.com/) 上，
喜欢折腾的可以自己按教程撸一遍，我之前也是在上面部署成功的。再次提醒账本是个人的隐私，
请注意个人隐私的保密。

### Vim snippets
使用文本编辑器编辑账本还有一个好处是你可以方便定制一些「模板」交易，比如每月例行的发工资，
交房租等，可以存成一个 code snippets，只需要改动一小部分即可。[这里](/wiki/language/vim.html)是我用到的一些 vim 下的 snippets，
使用前确保你装了`honza/vim-snippets`和`SirVer/ultisnips`插件。

[^1]: 你可以用`pushtag` `poptag` 语法省去每条交易都要打 tag 的麻烦
