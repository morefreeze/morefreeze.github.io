...
description: "用 in/out 编码把骑士巡游（哈密顿回路）转化为精准匹配问题"
category: algorithm
comments: true
...
8×8 棋盘上，有向封闭巡游的数量是 26,534,728,821,064 条（约 26.5 万亿），无向则减半。即使在小一点的 6×6 棋盘上，无向封闭巡游也有 9,862 条。
...
选出一组马步，让所有格子各有一进一出，就是精准匹配。代码实现非常干净（`knight_moves` 返回从给定格子出发的所有合法马步目标，完整实现见[文末链接][code]）：
...
例如在 6×6 棋盘上，DLX 找到的合法精准匹配里，绝大多数（超过 99.9%）是多环解，只有少数是真正的哈密顿回路。所以必须在重建路径时做一次检查：
...
5×5 棋盘的开放巡游，items 变成 52 个（$$2 \times 26$$），options 是 146 个（96 条真实马步 + 25 条虚拟出边 + 25 条虚拟入边）。
...
**两个封闭巡游算同一个**，当且仅当一个可以通过对称变换 + 改变起点 + 翻转方向变成另一个。要判断某条巡游在 180° 旋转下不变（`apply_symmetry` 对路径上每个格子施加对称变换，完整实现见[文末链接][code]）：
