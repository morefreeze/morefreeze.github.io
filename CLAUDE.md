...
### 数学表达式
- 本站使用 kramdown + MathJax 3 渲染数学公式
- **行内数学**：必须使用 `$$...$$`（双美元符号），不能用 `$...$`（单美元符号在 kramdown 中不生效）
- **独占行数学**：使用 `$$...$$` 独占一行
- 方程编号使用 `\tag{1}` 格式
- 示例：行内 `$$2^n$$`，独占行 `$$T(n) = 2^n - 1 \tag{1}$$`

### 图片
- 博客引用的图片必须实际存在于 `/images/` 目录下
- 如无对应图片，使用 ASCII art 或文字描述代替，不要引用不存在的图片文件
