...
- n=1：直接取下，1 步
- n=2：展示完整 2 步过程
- n=3：展示完整 5 步过程，用表格列出每步状态 (如 `111 → 110 → 100 → 101 → 001 → 000`)
...
把"环在杆上"记为 1，"不在"记为 0。列出 n=3 完整状态序列表格。

关键观察：**相邻两个状态之间，恰好只有一位不同。**

规则本身要求每步只操作一个环，所以这不是巧合。我们得到了一个遍历所有 3-bit 串、每次只改一位的序列。

### 4. 命名 — 这就是格雷码！ (~400字)

格雷码正式定义：2^n 个 n-bit 串的序列，相邻两串恰差一位。
